...
static GLboolean glmEqual(GLfloat* u, GLfloat* v, GLfloat epsilon) {
	if (glmAbs(u[0] - v[0]) < epsilon && glmAbs(u[1] - v[1]) < epsilon
...
static GLvoid glmReadMTL(GLMmodel* model, char* name) {
	...
	model->materials = (GLMmaterial*) malloc(
			sizeof(GLMmaterial) * nummaterials);
...
static GLvoid glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname) {
	...
	filename = (char*) malloc(
			sizeof(char) * (strlen(dir) + strlen(mtllibname)));
	strcpy(filename, dir);
	...
	while (fscanf(file, "%s", buf) != EOF) {
		...
		case 'v': /* v, vn, vt */
			...
			case '\0': /* vertex */
				...
				break;
			case 'n': /* normal */
				fscanf(file, "%f %f %f", &normals[3 * numnormals + 0],
						&normals[3 * numnormals + 1],
						&normals[3 * numnormals + 2]);
				numnormals++;
				break;
			case 't': /* texcoord */
				fscanf(file, "%f %f", &texcoords[2 * numtexcoords + 0],
...
GLvoid glmFacetNormals(GLMmodel* model) {
	...
	model->facetnorms = (GLfloat*) malloc(
			sizeof(GLfloat) * 3 * (model->numfacetnorms + 1));
...
GLvoid glmVertexNormals(GLMmodel* model, GLfloat angle) {
	...
	model->normals = (GLfloat*) malloc(
			sizeof(GLfloat) * 3 * (model->numnormals + 1));
...
GLvoid glmLinearTexture(GLMmodel* model) {
	...
	model->texcoords = (GLfloat*) malloc(
			sizeof(GLfloat) * 2 * (model->numtexcoords + 1));

	glmDimensions(model, dimensions);
	scalefactor = 2.0
			/ glmAbs(
					glmMax(glmMax(dimensions[0], dimensions[1]),
							dimensions[2]));
...
glmReadOBJ(const char* filename) {
	...
	/* allocate memory */
	model->vertices = (GLfloat*) malloc(
			sizeof(GLfloat) * 3 * (model->numvertices + 1));
	model->triangles = (GLMtriangle*) malloc(
			sizeof(GLMtriangle) * model->numtriangles);
////////////////////////////////////////////////////////////////////
	model->polygons = (GLMpolygon*) malloc(
			sizeof(GLMpolygon) * model->numpolygons);
	int num = model->numpolygons, i = 0;
	while (num-- > 0) {
		(model->polygons[i]).vindices = (GLuint*) malloc(
				sizeof(GLuint) * (int) ((float) model->numvertices + 1));
		(model->polygons[i]).nindices = (GLuint*) malloc(
				sizeof(GLuint) * (int) ((float) model->numnormals + 1));
		(model->polygons[i++]).tindices = (GLuint*) malloc(
				sizeof(GLuint) * (int) ((float) model->numtexcoords + 1));
		model->polygons[i].n = 0;
		model->polygons[i].t = 0;
	}
////////////////////////////////////////////////////////////////////
	if (model->numnormals) {
		model->normals = (GLfloat*) malloc(
	...
	if (mode & GLM_SMOOTH) {
		...
		for (i = 1; i <= model->numnormals; i++) {
			fprintf(file, "vn %f %f %f\n", model->facetnorms[3 * i + 0],
					model->facetnorms[3 * i + 1], model->facetnorms[3 * i + 2]);
		}
	}
